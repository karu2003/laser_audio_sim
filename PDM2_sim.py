...
def long_distance_photodetector(optical_signal):
    """
    Исправленная модель приема сигнала для больших расстояний.
    Убрана чрезмерная фильтрация перед компаратором.
    """
    ...
    # Добавляем шумы фотодиода (с уменьшенным влиянием)
    ...
    # ИСПРАВЛЕНО: Убираем предварительную фильтрацию сигнала перед компаратором,
    # которая слишком сильно сглаживала сигнал
    filtered_voltage = voltage_signal  # Используем исходный сигнал без фильтрации
    
    # ИСПРАВЛЕНО: Используем более простой метод определения порога,
    # основанный на глобальной статистике и гистограмме
    hist, bin_edges = np.histogram(filtered_voltage, bins=100)
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    
    # Находим два локальных максимума (для 0 и 1)
    # Это более надежно для PDM сигнала на больших расстояниях
    peaks, _ = signal.find_peaks(hist, height=np.max(hist)/10, distance=20)
    
    if len(peaks) >= 2:
        # Если нашли два пика - используем их для определения порога
        low_peak = bin_centers[peaks[0]]
        high_peak = bin_centers[peaks[-1]]
        threshold = (low_peak + high_peak) / 2
    else:
        # Запасной вариант - используем медиану и стандартное отклонение
        threshold = np.median(filtered_voltage) + 0.1 * np.std(filtered_voltage)
    
    # ИСПРАВЛЕНО: Компаратор без медианной фильтрации, которая могла удалять важные биты
    digital_output = (filtered_voltage > threshold).astype(float)
    
    return {
        'photocurrent': photocurrent,
        'voltage': voltage_signal,
        'filtered_voltage': filtered_voltage,  # теперь это просто копия voltage_signal
